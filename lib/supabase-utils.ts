...
import type { User, Room, Round, RoundResult, RoundHistoryItem, SessionUser } from "./types"
...
export async function joinRoom(roomId: string, name: string, authId?: string): Promise<{ userId: string }> {
  const supabase = createClientSupabaseClient()
  ...
  // Verificar se o usuário autenticado já está na sala
  if (authId) {
    ...
    if (existingUser) {
      // Usuário já está na sala, retornar o ID existente
      const sessionUser: SessionUser = {
        id: existingUser.id,
        roomId,
      }
...
export async function getCurrentUser(roomId: string): Promise<User | null> {
  const sessionUser = getSessionUser()

  if (!sessionUser || sessionUser.roomId !== roomId) {
    return null
  }
  ...
  return {
    id: userData.id,
    name: userData.name,
    isLeader: userData.is_leader,
    isObserver: userData.is_observer,
  }
}
